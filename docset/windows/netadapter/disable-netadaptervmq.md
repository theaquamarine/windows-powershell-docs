...
## DESCRIPTION
The **Disable-NetAdapterVmq** cmdlet disables the virtual machine queue (VMQ) feature on a network adapter.
VMQ is a scaling networking technology for the Hyper-V switch.
Without VMQ the networking performance of the Hyper-V switch bound to this network adapter may be reduced.

## EXAMPLES

### Example 1: Disable VMQ on the specified network adapter
```
PS C:\> Disable-NetAdapterVmq -Name "GuestTrafficAdapter"
```

This command disables VMQ on the network adapter named GuestTrafficAdapter and restarts the network adapter.
