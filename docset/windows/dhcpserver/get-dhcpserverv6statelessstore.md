...
If you do not specify the *Prefix* parameter, the properties of IPv6 stateless store at the server level are returned.

## EXAMPLES

### Example 1: Get server level configuration for stateless client store
```
...
### Example 2: Get configuration for stateless client store for a scope
```
...
### -Prefix
Specifies one or more IPv6 subnet prefixes for one or more scopes for which the stateless store properties are returned.
